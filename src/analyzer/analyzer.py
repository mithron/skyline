...
class Analyzer(Thread):
        ...
        for i, metric_name in enumerate(assigned_metrics):
            ...
            try:
                ...
                if anomalous:
                    ...
                    metric = [str(datapoint), base_name]
    ...
    def run(self):
        ...
        while 1:
			...
			##########################################################################################################################
			#add already existed metrics (to self.anomalous_metrics)
			##########################################################################################################################
            ...
            with open(filename, 'w') as fh:
                ...
                fh.write('%s' % str(anomalous_metrics).replace("'",'"'))
				
			#with open('webapp/static/dump/anomalies2.json', 'w') as fh:
                # Make it JSONP with a handle_data() function
                #anomalous_metrics = list(self.anomalous_metrics)
                #anomalous_metrics.sort(key=operator.itemgetter(1))
                #fh.write('%s' % anomalous_metrics)
